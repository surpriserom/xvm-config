 ...
 */
{
  "statisticForm": {
    ...
    "formatRightNick": "<textformat leading='9'><font color='{{c:kb}}' size='12'>{{kb%5.2f~k|----}}</font> <font color='{{c:wn8}}' size='12'>{{wn8|---}}</font> <font color='{{c:eff}}'>\u2588</font> <font size='12'>{{name%.15s~..}}</font><font alpha='#A0' size='10'>{{clan}}</font> <img src='xvm://res/icons/lang/{{region|CT}}/{{language|default}}.png' width='16' height='13'></textformat>",
    ...
    // Display format for the right panel (macros allowed, see readme-en.txt).
    "formatRightVehicle": "<textformat>{{vehicle}} <font color='{{c:t-battles}}' size='12'>{{t-battles|---}}</font> <font color='{{c:t-rating}}' size='12'>{{t-rating%2.01f~%|---}}</font></texformat>"
  }
}
